...
`1.0.0-beta.6`는 안정판 `1.0.0`으로 가기 위한 첫 공개 테스트 버전입니다.
베타 빌드는 프리릴리스이므로 릴리스 노트를 확인하고 자동 백업을 유지하며,
Windows·CPU 하드웨어 검증 결과에 따라 동작이 바뀔 수 있음을 전제로
사용하세요. 보호된 업데이터 서명 비밀 또는 저장소의 공개 키가 없으면 릴리스
자동화는 의도적으로 실패합니다.
